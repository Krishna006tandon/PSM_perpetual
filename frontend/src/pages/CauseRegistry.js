...
const CauseRegistry = ({ study, onBack, onNavigate, theme, toggleTheme , canEdit}) => {
  ...
  const fetchCauses = async () => {
    try {
      ...
      const colRes = await fetch(`https://api.perpetualsolutions.co.in/api/columns/${study._id}/causes`, {
      ...
      const response = await fetch(`https://api.perpetualsolutions.co.in/api/causes/${study._id}`, {
        headers: {
  ...
  const handleBlur = async (id, field, value, isCustom = false) => {
    try {
      ...
      await fetch(`https://api.perpetualsolutions.co.in/api/causes/${id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://api.perpetualsolutions.co.in/api/causes/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleMove = async (direction) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`https://api.perpetualsolutions.co.in/api/causes/reorder`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
