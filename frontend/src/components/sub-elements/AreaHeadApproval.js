...
const getStyles = (theme) => {
  ...
  return {
    ...
    riskBadge: (level) => {
      const colors = { High: ['#c5221f', '#fce8e6'], Medium: ['#b06000', '#fef7e0'], Low: ['#137333', '#e6f4ea'] };
      const [c, bg] = colors[level] || colors.Medium;
      return {
        display: 'inline-block', padding: '3px 10px', borderRadius: '20px', fontSize: '12px',
        fontWeight: '700', backgroundColor: isDark ? `${c}22` : bg, color: isDark ? `${c}dd` : c,
        border: `1px solid ${isDark ? `${c}44` : `${c}44`}`,
      };
    },
    ...
    textarea: {
      ...
      color: isDark ? '#e8e8ff' : '#1a1a3e',
      fontSize: '14px', outline: 'none', minHeight: '95px', width: '100%',
    ...
    descBox: {
      padding: '14px 16px', borderRadius: '8px', backgroundColor: isDark ? '#12122a' : '#f5f6fe',
      border: isDark ? '1px solid #2a2a4a' : '1px solid #e0e2f0',
      fontSize: '14px', color: isDark ? '#aab' : '#444', lineHeight: '1.6', marginBottom: '20px',
    },
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        <h3 style={styles.sectionTitle}>
          <span style={styles.stagePill}>2</span>
          Area Head Review
        </h3>
        ...
        {/* Action-specific banners — only shown after taking action */}
        {actionTaken === 'Approve' && (
          <>
            <div style={{ ...styles.alertBase, ...styles.alertSuccess }}>
              <span>✅</span>
              <span><strong>Approved!</strong> MOC has been approved and advanced to CTS Verification.</span>
            </div>
        ...
        {actionTaken === 'Query' && (
          <>
            <div style={{ ...styles.alertBase, ...styles.alertWarning }}>
              <span>❓</span>
              <span><strong>Query Raised.</strong> Your query has been logged and sent to the Process Engineer / Initiator.</span>
            </div>
            <div style={{ ...styles.alertBase, ...styles.alertApproved }}>
              <span>📬</span>
              <span>The query has been added to the Global Query Log. Please review other stages using the tracker above while awaiting a response.</span>
            </div>
          </>
        )}
        {actionTaken === 'Reject' && (
        ...
        <div style={styles.descBox}>
          <strong style={{ display: 'block', marginBottom: '6px', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Description</strong>
          {data.description}
        </div>
        ...
        {canAct && !actionTaken && (
          <>
            <div style={styles.inputGroup}>
              <label style={styles.inputLabel}>Reviewer Comments — Required for Query / Reject</label>
              <textarea style={styles.textarea} placeholder="Enter your observations or queries here..." value={comments} onChange={(e) => setComments(e.target.value)} />
            </div>
...
export default AreaHeadApproval;