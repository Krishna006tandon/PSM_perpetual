...
const getStyles = (theme) => {
  ...
  return {
    ...
    responsibilitiesBox: { padding: '14px 16px', borderRadius: '8px', backgroundColor: isDark ? '#12122a' : '#f0fff4', border: isDark ? '1px solid #2a2a4a' : '1px solid #ceead6', fontSize: '14px', color: isDark ? '#81c995' : '#137333', lineHeight: '1.6', marginBottom: '18px', borderLeft: '3px solid #137333' },
    archiveBox: { padding: '14px 16px', borderRadius: '8px', backgroundColor: isDark ? '#12122a' : '#f5f6fe', border: isDark ? '1px solid #2a2a4a' : '1px solid #e0e2f0', fontSize: '14px', color: isDark ? '#aab' : '#555', lineHeight: '1.6', marginBottom: '16px', borderLeft: '3px solid #137333' },
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        {/* Universal View */}
        {(!isWorkflowActive || !hasPermission) && !isCompleted && !actionTaken && (
        ...
        {actionTaken === 'Close' && (
          <>
            <div style={{ ...styles.alertBase, ...styles.alertSuccess }}>
              <span>🎉</span>
              <span><strong>MOC Successfully Closed!</strong> All workflows have been completed and verified. The MOC is now formally closed.</span>
            </div>
            <div style={{ ...styles.alertBase, ...styles.alertApproved }}>
              <span>✅</span>
              <span>Thank you for completing the final verification. The entire MOC lifecycle is now complete — you may review all stages using the tracker above.</span>
            </div>
          </>
        )}
        {actionTaken === 'Query' && (
          <>
            <div style={{ ...styles.alertBase, ...styles.alertWarning }}>
              <span>❓</span>
              <span><strong>Query Raised.</strong> Your query has been sent to the Project Manager / Initiator and logged.</span>
            </div>
            <div style={{ ...styles.alertBase, ...styles.alertApproved }}>
              <span>📬</span>
              <span>Review other stages using the tracker above while awaiting a response to your query.</span>
            </div>
          </>
        )}
        {actionTaken === 'Reject' && (
          <>
            <div style={{ ...styles.alertBase, ...styles.alertError }}>
              <span>⛔</span>
              <span><strong>Closure Rejected.</strong> The MOC closure has been rejected. Please review the query log and restart the verification phase.</span>
            </div>
            <div style={{ ...styles.alertBase, ...styles.alertWarning }}>
              <span>📋</span>
              <span>Your rejection has been recorded. Please coordinate with the Project Manager and team for corrective action.</span>
            </div>
          </>
        )}
        ...
        {isCompletedProp && (
          <div style={{ ...styles.alertBase, ...styles.alertSuccess }}>
            <span>🎉</span>
            <span>MOC Successfully Closed. All workflows completed and verified.</span>
          </div>
        )}
        ...
        {isCompletedProp && (
          <div style={styles.archiveBox}>
            <strong>Final Verification Remarks (Archived):</strong>
            <p style={{ marginTop: '6px', marginBottom: 0 }}>
              Final site walk-through completed. Physical modifications match MOC specifications. Operators informed and documentation received. Ticket formally closed.
            </p>
          </div>
        )}
        ...
        {canAct && !actionTaken && (
          <>
            <div style={styles.responsibilitiesBox}>
              <strong>Area Owner Responsibilities:</strong> Confirm that physical installation matches the approved MOC, site is clean and safe, operators are aware of changes, and documentation handover is complete.
            </div>
            <div style={styles.inputGroup}>
              <label style={styles.inputLabel}>Final Verification Remarks (Required)</label>
              <textarea style={styles.textarea} placeholder="State verification findings (e.g., 'Site walked through, no leaks found, operators trained. Closing ticket.')" value={comments} onChange={(e) => setComments(e.target.value)} />
            </div>
            <div style={styles.buttonGroup}>
              <button style={styles.btnApprove} onClick={() => handleAction('Close')} disabled={isProcessing}>{isProcessing ? '⏳ Processing...' : '✓ Approve'}</button>
